...
const Dashboard: React.FunctionComponent<DashboardProps> = ({
  ...
  const [resourceList, setResourceList] = useState<ResourceConfig[]>(
    resources || defaultConfig.resources,
  );
  const [modalOpen, setModalOpen] = useState(false);
  const [fieldsModalOpen, setFieldsModalOpen] = useState(false);
  const [activeResourceIdx, setActiveResourceIdx] = useState<number | null>(
    null,
  );
      ...
      prev.map((res, idx) =>
        idx === activeResourceIdx
          ? {
              ...
              fields: [
                ...res.fields,
                { ...newField, type: newField.type as FieldType },
              ],
            }
          : res,
      ),
  ...
  return (
    <div className="p-4 bg-glam min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-extrabold tracking-tight text-blue-800 drop-shadow">
          Resources
        </h1>
        <button
      ...
      >
        <form onSubmit={handleAddResource} className="space-y-4">
          ...
          <button
            type="submit"
            className="bg-blue-600 text-white px-4 py-2 rounded font-semibold w-full hover:bg-blue-700 transition"
          >
            Add Resource
          </button>
        </form>
          ...
          <div>
            ...
            <form onSubmit={handleAddField} className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="submit"
                  className="ml-auto bg-blue-600 text-white px-3 py-1 rounded font-semibold hover:bg-blue-700 transition text-sm"
                >
                  Add Field
                </button>
              </div>
