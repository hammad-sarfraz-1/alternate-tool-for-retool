        ...
        <div className="flex-1 flex flex-col min-h-screen">
          <main className="flex-1 max-w-5xl mx-auto w-full py-10 px-6">
            <Routes>
              ...
              <Route path="/app/:appId" element={<AppModulesPage />} />
              <Route
                path="/app/:appId/module/:moduleId"
                element={<ModuleEntityPage />}
              />
              {/* legacy routes below, to be refactored later */}
              <Route path="/dashboard" element={<DashboardWithRouting />} />
              {resourceRoutes.map((r) => (
                <Route
                  key={r.name}
                  path={`/${r.name}`}
                  element={<ResourcePageWithRouting resourceName={r.name} />}
                />
              ))}
          ...
          <footer className="text-center text-xs text-gray-400 py-4 border-t bg-white">
            &copy; {new Date().getFullYear()} Teacher Training Platform. Crafted
            with{" "}
      ...
      <div className="space-x-4">
        {resourceRoutes.map((r) => (
          <Link
            key={r.name}
            to={`/${r.name}`}
            className={`text-blue-600 hover:underline font-medium ${currentPath === r.name ? "underline" : ""}`}
          >
            {r.label}
          </Link>
        ))}
        <Link
          to="/settings"
          className={`text-blue-600 hover:underline font-medium ${currentPath === "settings" ? "underline" : ""}`}
        >
          Settings
        </Link>
      </div>
